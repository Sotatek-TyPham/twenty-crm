...
setupDB:
	docker compose exec node npx nx database:reset twenty-server
migrate:
	nx run twenty-server:typeorm migration:run -d src/database/typeorm/core/core.datasource
startServer:
	docker compose exec node npx nx start twenty-server
startWorker:
	docker compose exec node npx nx worker twenty-server
startFront:
	docker compose exec node npx nx start twenty-front
startAll:
	docker compose exec node npx nx start
